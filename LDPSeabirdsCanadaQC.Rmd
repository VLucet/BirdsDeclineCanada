...
QCSB_table %>%
  ...
  cols_label(espece_species_en = "Species", percent_change = html("Percent Change in Quebec<br>(1970 - 2016)"), status = "Canada", status_QC = "Quebec") %>%
  data_color(apply_to = "text", columns = vars(percent_change), colors = scales::col_bin(palette = c("firebrick", "slategray3", "chartreuse3"), bins = c(min(percent_change),-range,range,max(percent_change))))
```
